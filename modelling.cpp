 ...
 * Главные действия: удалить индекс столбец, если он есть; извлечь целевой столбец, обучить модель
 ...
 * @return accuracy Точность
 */

double CalculateAccuracy(const arma::rowvec& predictions, const arma::rowvec& actual) {
    int correct = 0;
    for (size_t i = 0; i < predictions.n_elem; ++i)
        if (round(predictions[i]) == actual[i])
            correct++;
    return 1.0 * correct / predictions.n_elem;
}
