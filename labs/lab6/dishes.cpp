...
int maxSatisfaction(std::vector<int>& satisfaction) { // +24 байта на вектор
    ...
    for (int& sat_coef : satisfaction) { // O(N), проход по всем элементам
        ...
        // Если же сумма положительная, то мы можем добавить еще блюдо (даже если у него отрицательный коэф), его мы добавляем в начало очереди (все прошлые готовятся вслед за ним), тогда к результату прибавится сумма всех коэффициентов, тк время готовки каждого прошлого блюда увеличится ровно на 1 и добавится нового блюдо (его время будет равным 1)
