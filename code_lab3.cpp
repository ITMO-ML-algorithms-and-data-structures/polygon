...
/* Задача: сэмплирование (+ с большими данными до 1e9 эл-ов в массиве)*/
...
long int IndexRandom(long int max) {
    ...
    if (indexes_map.find(new_index) == indexes_map.end()) { // проверяем что такого индекса ещё не встречалось
        return new_index;
    } // O(1) в среднем тк поиск в unordered_map
    return -1;
}
...
std::vector<long int> GoAndSample(std::vector<long int> given_massive, long int n) {
    ...
    // берём n "случайных" индексов, по ним значения добавляем в arr
    for (long int i = 0; i < n; ++i) {
        long int new_index = IndexRandom(given_massive.size()); // генерация "случайного" индекса (O(1))
        if (new_index > -1) { // O(1) 
            massive_sampled.push_back(given_massive[new_index]); // O(1) 
        }
    }
    return massive_sampled; // O(1) + O(n), в любом случае временная сложность будет линейной 
}
...
bool containsAll(const std::vector<long int>& sampledarr, const std::vector<long int>& bigarr) {
    for (const auto& elem : sampledarr) {
        bool found = false;
        for (const auto& sup_elem : bigarr) {
            if (elem == sup_elem) {
                found = true;
                break;
            }
