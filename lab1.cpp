...
// Для хеш таблицы учитываются примерные значения, так как сильно от компилятора и системы

int main(int argc, char* argv[]) { // В функцию main передается количество аргументов и сами аргументы командной строки
    ifstream file(argv[1]);  // Открываем файл

    int size;
    file >> size;  // Ввод размера
    ...
    for (int i = 0; i < size; i++) {
        ...
        ans.insert(elem);  // Добавление в хеш таблицу
    }
    
    for (int elem : ans) {
        cout << elem << ' ';  // Вывод элементо
    } cout << '\n';
