...
std::vector<unsigned char> label_encoder(const std::string& path) {    // Функция принимает один аргумент - путь до файла
    ...
    std::string inp;    // Создаем переменную для хранения считанных из файла строк, память - (k), где K - кол-во эл-тов строки
    unsigned char next_value = 1;    // Переменная для хранения номера первый раз встреченной строки, память - (1)
