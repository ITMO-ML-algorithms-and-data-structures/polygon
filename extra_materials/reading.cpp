...
template <typename T>
std::vector<std::vector<T>> read_csv(const char filename[],
                                     const char delimiter = ',',
                                     bool skip_header = true) {
  // Эта функция нужно для задач машинного обучения уровня hard+, игнорируйте ее
  // если обратного не сказано
...
void run_tests() {
  ...
  std::vector<int> vec_int =
      read_array_from_file<int>("../data/tests/vector1d_int.csv");
  ...
  std::vector<float> vec_float =
      read_array_from_file<float>("../data/tests/vector1d_float.csv");
  std::vector<float> vec_float_true = {1.1, 1.2, 2.3, 3.4};
  assert(
      std::equal(vec_float.begin(), vec_float.end(), vec_float_true.begin()));

  // Проверка что массив с строками числами считывается корректно
  std::vector<std::string> vec_str =
      read_array_from_file<std::string>("../data/tests/vector1d_str.csv");
